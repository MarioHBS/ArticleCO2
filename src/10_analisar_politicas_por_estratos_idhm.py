...
=======
    ...
    if 'idhm_' in df.columns and df['idhm_'].sum() > 0:
        ...
        df_estratos['estrato_desenvolvimento'] = df_estratos['pib_per_capita'].apply(classificar_estrato_pib)
        idhm_col = 'pib_per_capita'
...
def analisar_efetividade_por_estrato(df_estratos):
    ...
    for estrato in df_valido['estrato_desenvolvimento'].unique():
        ...
        df_estrato_pib = df_estrato[df_estrato['pib'] > 0]
        if len(df_estrato_pib) > 0:
            metricas['intensidade_carbono'] = (df_estrato_pib['GEE_tCO2e'] / df_estrato_pib['pib']).mean()
            metricas['intensidade_carbono_std'] = (df_estrato_pib['GEE_tCO2e'] / df_estrato_pib['pib']).std()
...
def gerar_visualizacoes_estratos(df_estratos, resultados):
    ...
    plt.tight_layout()
    plt.savefig('results/figures/Figura13_Analise_Estratos_Desenvolvimento.png', 
...
=======
            columns=['Desmatamento\nMédio (ha)', 'Emissões\nMédias (tCO2e)', 
                    'Intensidade\nCarbono', 'Tendência\nDesmatamento', 'Tendência\nEmissões']
        )
        
        # Normalizar dados para melhor visualização
        df_heatmap_norm = df_heatmap.div(df_heatmap.abs().max(), axis=1)
        
        sns.heatmap(df_heatmap_norm, annot=True, cmap='RdYlBu_r', center=0, 
                   fmt='.2f', cbar_kws={'label': 'Valores Normalizados'})
        plt.title('Índice de Causalidade: Métricas Ambientais por Estrato de Desenvolvimento')
        plt.ylabel('Estrato de Desenvolvimento')
        plt.xlabel('Métricas Ambientais')
        
        plt.tight_layout()
        plt.savefig('results/figures/Figura14_Heatmap_Metricas_Estratos.png', 
                    dpi=300, bbox_inches='tight')
        plt.close()
...
def gerar_relatorio_estratos(resultados):
    ...
    for estrato, metricas in resultados.items():
        ...
        relatorio.append(f"Desmatamento médio: {metricas.get('desmatamento_medio', 0):.2f} ± {metricas.get('desmatamento_std', 0):.2f} ha")
        relatorio.append(f"Emissões médias: {metricas.get('emissoes_medias', 0):.2f} ± {metricas.get('emissoes_std', 0):.2f} tCO2e")
        relatorio.append(f"PIB médio: R$ {metricas.get('pib_medio', 0):,.2f} ± {metricas.get('pib_std', 0):,.2f}")
        
        if 'intensidade_carbono' in metricas:
            relatorio.append(f"Intensidade de carbono: {metricas['intensidade_carbono']:.6f} ± {metricas.get('intensidade_carbono_std', 0):.6f} tCO2e/R$")
        
        if 'tendencia_desmatamento' in metricas:
            relatorio.append(f"Tendência de desmatamento: {metricas['tendencia_desmatamento']:.2f} ha/ano (R² = {metricas.get('r2_desmatamento', 0):.3f})")
            relatorio.append(f"Tendência de emissões: {metricas['tendencia_emissoes']:.2f} tCO2e/ano (R² = {metricas.get('r2_emissoes', 0):.3f})")
    ...
    # Ranking por desmatamento
    estratos_desmat = [(estrato, metricas.get('desmatamento_medio', 0)) 
                      for estrato, metricas in resultados.items()]
    estratos_desmat.sort(key=lambda x: x[1], reverse=True)
...
=======
    ...
    # Ranking por intensidade de carbono
    estratos_intensidade = [(estrato, metricas.get('intensidade_carbono', 0)) 
                           for estrato, metricas in resultados.items() 
                           if 'intensidade_carbono' in metricas]
    estratos_intensidade.sort(key=lambda x: x[1], reverse=True)
...
=======
    ...
    # Salvar relatório
    with open('results/relatorio_analise_estratos_desenvolvimento.txt', 'w', encoding='utf-8') as f:
        f.write('\n'.join(relatorio))
    
    print("Relatório salvo em: results/relatorio_analise_estratos_desenvolvimento.txt")
...
def main():
    ...
    try:
        ...
        print("=" * 80)
        print("\nArquivos gerados:")
        print("- results/figures/Figura13_Analise_Estratos_Desenvolvimento.png")
        print("- results/figures/Figura14_Heatmap_Metricas_Estratos.png")
